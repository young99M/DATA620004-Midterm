...
def train(epoch):
    ...
    for batch_index, (images, labels) in enumerate(cifar100_training_loader):
        ...
        loss_mix_up = mixup_criterion(labels_a, labels_b, lam)
        loss = loss_mix_up(loss_function, outputs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-type', type=str, default='mixup', help='baseline or not')
    ...
    if epoch > milestones[0] and best_acc < acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt_mixup.pth')
        best_acc = acc
