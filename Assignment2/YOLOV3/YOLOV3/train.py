...
class Trainer(object):
    def __init__(self,  weight_path, resume, gpu_id):
        ...
        self.test_dataset = data.VocDataset(anno_file_type="test", img_size=cfg.TEST["TEST_IMG_SIZE"])
        self.test_dataloader = DataLoader(self.test_dataset, batch_size=cfg.TEST["BATCH_SIZE"], 
                                        num_workers=cfg.TEST["NUMBER_WORKERS"], shuffle=True)
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.train_dataloader):
                ...
                if i%10==0:
                    ...
                    print(s)
                    writer.add_scalar('train loss iter/giou', mloss[0], global_step=len(self.train_dataloader)*epoch + i)
                    writer.add_scalar('train loss iter/class', mloss[2], global_step=len(self.train_dataloader)*epoch + i)
                    # writer.add_scalar('train_loss_iter', mloss[3], global_step=i)
            ...
            writer.add_scalars('Train val loss/giou', {'train loss': mloss[0]}, global_step=epoch+1)
            writer.add_scalars('Train val loss/cls', {'train loss':mloss[2]}, global_step=epoch+1)
            # writer.add_scalar('train loss', mloss[3], global_step=epoch)
            
            tloss = torch.zeros(4)
            count = 0
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.test_dataloader):
                count += 1
                if count>500:
                    continue
                imgs = imgs.to(self.device)
                ...
                loss, loss_giou, loss_conf, loss_cls = self.criterion(p, p_d, label_sbbox, label_mbbox,
                                                  label_lbbox, sbboxes, mbboxes, lbboxes)
                loss_items = torch.tensor([loss_giou, loss_conf, loss_cls, loss])
                tloss = (tloss * i + loss_items) / (i + 1)
                
            writer.add_scalars('Train val loss/giou', {'test loss': tloss[0]}, global_step=epoch+1)
            writer.add_scalars('Train val loss/cls', {'test loss':tloss[2]}, global_step=epoch+1)
            # writer.add_scalar('train loss', tloss[3], global_step=epoch)
            ...
            with torch.no_grad():
                ...
                writer.add_scalar('test mAP', mAP, global_step=epoch+1)
                print('mAP:%g'%(mAP))
