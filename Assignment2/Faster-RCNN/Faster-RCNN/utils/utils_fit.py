...
def fit_one_epoch(model, train_util, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir):
    ...
    if (epoch + 1) % save_period == 0 or epoch + 1 == Epoch:
        model_path = os.path.join(save_dir, 'ep%03d-loss%.3f-val_loss%.3f.pth' % (epoch + 1, total_loss / epoch_step, val_loss / epoch_step_val))
        torch.save(model.state_dict(), model_path)
        # print('Computing mAP')
        # loss_history.update_map(epoch + 1, model_path, MINOVERLAP=0.5)
